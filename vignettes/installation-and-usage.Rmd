...
### 2. Create a Database and Connection

First, specify a path for your database and create a database object. Then, establish a connection to it. #TODO add mention of in memory or path
Copy nodes before relationships: In order to copy a relationship table R from a Parquet file RFile, the nodes that appear in RFile need to already exist in the database (either imported in bulk or inserted through Cypher data manipulation commands).
...
kuzu_execute(con, "INSTALL duckdb;")
...
```{r}
# Find relationships
result_rel <- kuzu_execute(con, "MATCH (a:Person)-[k:Knows]->(b:Person) RETURN a.name, k.since, b.name")
