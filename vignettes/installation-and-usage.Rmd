...
This vignette provides a guide to `kuzuR` and performing basic operations, including creating a database, defining a schema, loading data from various sources, and executing Cypher queries.

## 1. Connecting to a Database

First, load the `kuzuR` package and create a connection to a Kuzu database. You can create an in-memory database or specify a path to a database file on disk.
...
Kuzu supports a rich set of data types. When creating a schema, you need to map your R data types to the corresponding Kuzu `LogicalTypeID`.
...
You can define a schema with node and relationship tables using `kuzu_execute()`. Here's an example of a more complex schema:
...
You can load data into your Kuzu tables from R data frames or external files like CSV.

### Loading from a Data Frame

Use `kuzu_copy_from_df()` to load data from an R `data.frame`.
...
Use `kuzu_copy_from_csv()` to load data from a CSV file. For this to work, the file should be in the current working directory.
...
You can execute Cypher queries using `kuzu_execute()` and convert the results into various R formats.
...
The `QueryResult` object returned by `kuzu_execute()` acts as an iterator over the results. This means it can only be consumed once. Functions like `as.data.frame()`, `as_tibble()`, `kuzu_get_all()`, and the graph conversion functions will exhaust this iterator.
