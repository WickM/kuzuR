...
kuzu_execute(con, paste("CREATE NODE TABLE User(userID UUID, name STRING,",
                        ...
                        "PRIMARY KEY (userID))"))
#> <kuzu.query_result.QueryResult object at 0x7fee379d4440>
...
#> <kuzu.query_result.QueryResult object at 0x7fee366accb0>
...
#> <kuzu.query_result.QueryResult object at 0x7fee366ad310>
```
...
print(igraph_obj)
#> IGRAPH 95ebea7 DN-- 4 2 -- 
#> + attr: name (v/c), userID (v/x), age (v/n), is_active (v/x),
#> | created_at (v/n), last_login (v/x), notes (v/x), User (v/x), label
#> | (v/c), productID (v/n), Product (v/x), _dst (e/x), purchase_date
#> | (e/x), _label (e/c), _id (e/x), _src (e/x)
#> + edges from 95ebea7 (vertex names):
...
#> #
#> # Edge Data: 2 × 7
#>    from    to `_dst`           purchase_date `_label` `_id`        `_src`      
#>   <int> <int> <list>           <list>        <chr>    <list>       <list>      
#> 1     1     2 <named list [2]> <date [1]>    Buys     <named list> <named list>
#> 2     3     4 <named list [2]> <date [1]>    Buys     <named list> <named list>
