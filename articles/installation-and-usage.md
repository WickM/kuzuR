...
kuzu_execute(con, paste("CREATE NODE TABLE User(userID UUID, name STRING,",
                        ...
                        "PRIMARY KEY (userID))"))
#> <kuzu.query_result.QueryResult object at 0x7f80f91d6060>
...
#> <kuzu.query_result.QueryResult object at 0x7f8108185100>
...
#> <kuzu.query_result.QueryResult object at 0x7f80f90ad340>
```
...
print(igraph_obj)
#> IGRAPH 3b8932b DN-- 4 2 -- 
#> + attr: name (v/c), userID (v/x), age (v/n), is_active (v/x),
#> | created_at (v/n), last_login (v/x), notes (v/x), User (v/x), label
#> | (v/c), productID (v/n), Product (v/x), _dst (e/x), _label (e/c), _id
#> | (e/x), purchase_date (e/x), _src (e/x)
#> + edges from 3b8932b (vertex names):
...
#> #
#> # Edge Data: 2 × 7
#>    from    to `_dst`           `_label` `_id`        purchase_date `_src`      
